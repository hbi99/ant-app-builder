...
const buildTasks = async (appXmlDom, appPath, id, namespace, uglify) => {
	...
	return Promise.all(buildTasks.map(async task => {
		let filePath = FS.path.join(appPath, task._attributes.source)
		...
		let requireFile = async (base, importStr, ignore=[]) => {
			...
			if (exists) {
				...
				repl = repl.toString().replace(/\$/g, "$::")
				// replace with content
				let rx = new RegExp(importStr, "i")
				code = code.replace(rx, repl).replace(/\$\:\:/g, "$")
			}
		...
		requires.map(path => {
			let rx = new RegExp(path, "i")
			let repl = `(() => {throw "File not found: ${path}";})()`
		...
		if (_uglify) {
			// terser.js
			let options = { compress: false, module: false, mangle: _uglify }
			let parsed = terser.minify(code, options)
			await FS.writeFile(dest, parsed.code)
			return
		}
...
const compileScript = async (meta, appPath, uglify) => {
	...
	requires.map(path => {
		let rx = new RegExp(path, "i")
		let repl = `(() => {throw "File not found: ${path.slice(9,-1)}";})()`
		code = code.replace(rx, repl)
	})
